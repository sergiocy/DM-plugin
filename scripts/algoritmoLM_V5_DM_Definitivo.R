...
datosEntrada <- function(){
  ...
  ESTIMADORES_INICIALES <<- ESTIMADORES_I
  
  write.table(as.matrix(ESTIMADORES_INICIALES),"C:\\Users\\Usuario\\Desktop\\plugInDM\\estimInicialesLM.txt", row.names=F, col.names=F)
...
algoritmoLM <- function(tablaDatos){ 
 ...
 for(i in 1:NUM_COMPONENTES){ 
  ...
  cotaSuperior <- c(cotaSuperior,ESTIMADORES_INICIALES[i,1]+7,ESTIMADORES_INICIALES[i,2]+7,ESTIMADORES_INICIALES[i,3]+7,ESTIMADORES_INICIALES[i,4]+7,0.99,3/NUM_COMPONENTES)
  cotaInferior <- c(cotaInferior,ESTIMADORES_INICIALES[i,1]-7,ESTIMADORES_INICIALES[i,2]-7,0.1,0.1,-0.99,0.01/NUM_COMPONENTES)
 }
