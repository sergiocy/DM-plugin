...
algoritmoLM <- function(tablaDatos){ 
 ...
 for(i in 1:NUM_COMPONENTES){ 
  ...
  cotaSuperior <- c(cotaSuperior,ESTIMADORES_INICIALES[i,1]+7,ESTIMADORES_INICIALES[i,2]+7,ESTIMADORES_INICIALES[i,3]+7,ESTIMADORES_INICIALES[i,4]+7,0.99,3)
 ...
 ajuste <- nls.lm(par=param_iniciales, upper=cotaSuperior, lower=cotaInferior, fn=residuales, x=tablaDatos$x, y=tablaDatos$y, z=tablaDatos$z,
                  control=nls.lm.control(nprint=1, factor=100), jac=calculaGradiente2 )
